...
const showFooter = computed(() => {
  if (isMobile.value)
    return !route.name?.toString().includes('casino-games') && !route.name?.toString().includes('casino-original-game')
  return true
})
...
<template>
  ...
  >
    ...
    <div id="TgMainContent" class="main-content" :class="{ 'not-mobile': !isMobile, 'content-top': isMobile }">
      <header class="navigation" :class="{ 'h5-pos': isMobile }">
        <AppContent except>
          <AppHeader />
        </AppContent>
      </header>
