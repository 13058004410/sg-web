...
const phoneErrorMsg = computed(() => {
  if (!paramsData.value.phone)
    return ''
  return numberReg.test(paramsData.value.phone) && paramsData.value.phone.length > 5 ? '' : t('phone_invalid')
})
...
watch(areaCodeData, (newVal) => {
  if (!paramsData.value.area_code)
    paramsData.value.area_code = newVal ? newVal[0].id : ''
