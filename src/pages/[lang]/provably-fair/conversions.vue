...
<template>
  <div>
    ...
    <div class="text-tg-text-white scroll-x bg-tg-secondary-grey mt-16 rounded-[4px] p-16 text-[16px] leading-[1.5] md:text-[18px]">
...
function generateFloats ({ serverSeed, clientSeed, nonce, cursor, count }) {
  ...
  return _.chunk(bytes, 4).map(bytesChunk =>
    ...
    <div class="text-tg-secondary-light mt-16 text-[16px] leading-[1.5] md:text-[18px]">
      For games such as Keno, Mines and Video Poker, where outcomes cannot be duplicated, we then utilise the <a class="text-tg-text-white center inline-flex gap-4 text-[14px]" href="https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle" target="_blank">Fisher-Yates shuffle <BaseIcon class="text-[12px]" name="uni-jump-page" /></a> algorithm. This procedure influences the conversion process from floats to game events because each time a game event is translated, the amount of possible remaining game event possibilities has been reduced for any remaining steps in the result chain.
    </div>
    <div class="text-tg-secondary-light mt-16 text-[16px] leading-[1.5] md:text-[18px]">
