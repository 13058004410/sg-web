...
export const useAppStore = defineStore('app', () => {
  ...
  const companyData = ref({
    email: `support@${VITE_OFFICIAL_DOMAIN}`,
    // 合作伙伴邮箱
  ...
  const logoAndIcoAndLoading = computed(() => {
    if (brandPcDetail.value) {
      return {
        logo_white: brandPcDetail.value.logo_white,
        logo_gray: brandPcDetail.value.logo_gray,
        ico: brandPcDetail.value.icon || '',
        loadingImgUrl: brandPcDetail.value.loading?.image ?? '',
      }
    }
    return {
      logo_white: '',
