...
export function useApiMenuData() {
  ...
  const casinoGameProvider = computed<Menu>(() => [{ title: t('h5_casino_browse_pop_up_menu_game_provider'), path: '/casino/collection/provider', icon: 'chess-game-provider', list: [], domId: '' }])
  ...
  const staticMenu1 = computed(() => <Menu>[
    ...
    //   icon: 'chess-gameshow',
    //   list: [
    //     { title: '免费获得100钱币', path: '/promotions/promotion/weekly-giveaway', icon: 'uni-roulette' },
