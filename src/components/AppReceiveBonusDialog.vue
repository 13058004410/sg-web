...
const rate = computed(() => {
  return getRate('USDT', activeCurrency.value?.type ?? 'USDT', 8)?.targetNum
...
const ratedMoney = computed(() => {
  if (rate.value && activeCurrency.value)
    return toFixed(+mul(+money.value, +rate.value), application.isVirtualCurrency(activeCurrency.value.type) ? 8 : 2)
})
