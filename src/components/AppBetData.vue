...
const date = {
  start_time: dayjs(new Date()).startOf('day').format('YYYY-MM-DD HH:mm:ss'),
  end_time: dayjs(new Date()).endOf('day').format('YYYY-MM-DD HH:mm:ss'),
}
const { isLogin } = storeToRefs(useAppStore())
const { isLessThanLg, isGreaterThanSm } = storeToRefs(useWindowStore())
const { bool: color, setBool: setColor } = useBoolean(false)
// 定时器开启时，关闭loading
const { bool: timeLoad, setBool: setTimeLoad } = useBoolean(false)
...
const getTableColumns: ComputedRef<RewriteColumn[]> = computed((): RewriteColumn[] => {
  switch (activeTab.value) {
    ...
    case 'casino-fy': return [
      ...
      {
        title: t('h5_bet_bottom_menu_bar_slip_table_header_payout'),
        width: '16%',
        dataIndex: 'net_amount',
        slot: 'payMoney',
        align: 'right',
...
watch(() => selectSize.value, (newValue) => {
  if (activeTab.value === 'casino-mine') {
    ...
    !rankList.value.length && runMemberCompetitionList(date)
  }
  else { betPageSize.value = Number(newValue) }
})
    ...
    <template v-if="selectSize !== '0'">
      <div v-show="activeTab === 'ranking-list'" class="ranking-time">
        <div class="center cursor-pointer">
          ...
          <span>{{ t('competition', { money: `$${application.numberToLocaleString(100000, getCurrentLanguageForFrontend())}` })
          }} – {{
      ...
      <BaseTable
        ...
        :data-source="getList"
        :style="getBgColor"
        :loading="(loading || loadBet || loadRank) && !timeLoad"
        :last-first-padding="isMobile"
        :show-empty="false"
        is-amount-popper
      >
        <template #gameName="{ record }">
          <div class="game-box" @click="mode === 'casino' && openBetSlipDialog({ type: 'casino', data: record })">
            <AppImage :url="`${record.icon}`" err-icon="chess-slot-machine" width="14px" is-network />
            <div class="game-name">
              {{ record.game_name }}
            </div>
          </div>
