...
const active = computed(() => {
  ...
  const _path = `/${getCurrentUrlLanguage()}${props.menuItem.path}`
...
<template>
  <div class="tg-app-menu-item" :class="[menuItem.clazz]">
    <div
      class="menu-item" :class="{ active, disabled: menuItem.token && !isLogin }"
      style="--app-sport-image-error-icon-size:14px;"
    >
      <AppImage
        v-if="menuItem.useCloudImg" style="width: 14px;height: 14px;" :url="menuItem.icon" is-cloud
        class="icon-img"
      />
      <BaseIcon v-else-if="menuItem.icon" :name="menuItem.icon" />
