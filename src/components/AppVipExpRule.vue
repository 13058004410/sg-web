...
const tableData = computed(() => {
  if (data.value) {
    return expData.value.map((a) => {
      ...
      return {
        ...a,
        sports: toFixed(+a.exp * +sportsRate, 8).toString().slice(0, 10) + t('vip_promotion_bonus_table_header_integral'),
        casino: toFixed(+a.exp * +casinoRate, 8).toString().slice(0, 10) + t('vip_promotion_bonus_table_header_integral'),
        slot: toFixed(+a.exp * +slotRate, 8).toString().slice(0, 10) + t('vip_promotion_bonus_table_header_integral'),
        fish: toFixed(+a.exp * +fishRate, 8).toString().slice(0, 10) + t('vip_promotion_bonus_table_header_integral'),
      }
    })
  }
  return []
