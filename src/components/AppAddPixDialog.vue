...
const validationSchema = yup.object({
  open_name: yup.string().trim().required(t('enter_your_content')),
  ...
  pay_password: yup.string().trim().required(t('enter_your_content')).test('pay_password', t('wrong_format'), (val) => {
    return payPasswordReg.test(val)
  }),
})
// #endregion
...
const { value: open_name, errorMessage: open_nameError } = useField<string>('open_name')
...
onMounted(() => {
  if (props.openName)
    open_name.value = props.openName
})
  ...
  <div class="px-16 pb-16">
    <form class="border-tg-secondary border rounded-[4px] border-solid" @submit="onSubmit">
      ...
      <div class="flex gap-14 px-20 pt-14">
        <div class="flex-1">
          <BaseLabel must :label="$t('account_open_name')">
            <BaseInput v-model="open_name" :msg="open_nameError" :disabled="!!openName" />
          </BaseLabel>
        </div>
      </div>
      <div class="flex gap-14 px-20 pt-14">
