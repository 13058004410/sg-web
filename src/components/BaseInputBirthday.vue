  ...
  <div class="base-input-birthday">
    <div :class="[layout]">
      ...
      <div class="input-wrap">
        <!-- 日 -->
        <input
          ref="dayInputRef" v-model="day" type="number" :min="1" :max="dayMax" autocomplete="on"
          :placeholder="t('reg_day')" :class="{ error: errorDayMsg || showAllRed }" @input="onInput"
        >
        <!-- 月 -->
        <div class="select-wrap">
          <select
            v-model="month" required :class="{
        ...
        <!-- 年 -->
        <input
          ref="yearInputRef" v-model="year" :class="{ error: errorYearMsg || showAllRed }" type="number" :min="1900"
          placeholder="YYYY" @input="onInput"
        >
      </div>
    </div>
